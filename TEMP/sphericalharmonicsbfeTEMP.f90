MODULE sphericalharmonicsbfe
    ...
    REAL*8, DIMENSION(:,:,:), ALLOCATABLE, PUBLIC :: BASIS_RHO_L_COMPONENT_GRID
    REAL*8, DIMENSION(:,:,:), ALLOCATABLE, PUBLIC :: BASIS_PHI_L_COMPONENT_GRID
    REAL*8, DIMENSION(:,:,:), ALLOCATABLE, PUBLIC :: BASIS_DPHI_L_DR_COMPONENT_GRID
...
CONTAINS
    ...
    SUBROUTINE clearsphericalharmonicbasis()
        ...
        IF (ALLOCATED(BASIS_RHO_L_COMPONENT_GRID)) DEALLOCATE(BASIS_RHO_L_COMPONENT_GRID)
        IF (ALLOCATED(BASIS_PHI_L_COMPONENT_GRID)) DEALLOCATE(BASIS_PHI_L_COMPONENT_GRID)
        IF (ALLOCATED(BASIS_DPHI_L_DR_COMPONENT_GRID)) DEALLOCATE(BASIS_DPHI_L_DR_COMPONENT_GRID)
    ...
    SUBROUTINE initsphericalharmoniccomponentstorage(ncomp)
        IMPLICIT NONE
        INTEGER, INTENT(IN) :: ncomp

        IF (.NOT. BASIS_GRID_SET) THEN
            WRITE(*,'(A)') "WARNING: initsphericalharmoniccomponentstorage: initialize basis grid first"
            RETURN
        END IF
        IF (ncomp < 1) THEN
            IF (ALLOCATED(BASIS_RHO_L_COMPONENT_GRID)) DEALLOCATE(BASIS_RHO_L_COMPONENT_GRID)
        ...
        ALLOCATE(BASIS_RHO_L_COMPONENT_GRID(0:BASIS_LMAX, BASIS_NR, ncomp))
        ALLOCATE(BASIS_PHI_L_COMPONENT_GRID(0:BASIS_LMAX, BASIS_NR, ncomp))
        ALLOCATE(BASIS_DPHI_L_DR_COMPONENT_GRID(0:BASIS_LMAX, BASIS_NR, ncomp))

        BASIS_RHO_L_COMPONENT_GRID = 0.0D0
        BASIS_PHI_L_COMPONENT_GRID = 0.0D0
        BASIS_DPHI_L_DR_COMPONENT_GRID = 0.0D0
        BASIS_NCOMP = ncomp
    END SUBROUTINE initsphericalharmoniccomponentstorage

    SUBROUTINE project_axisymmetric_density_to_table(params, density_model, rho_l_out)
        ! Generic Legendre projection for any axisymmetric density model.
        IMPLICIT NONE
        REAL*8, INTENT(IN), DIMENSION(:) :: params
        PROCEDURE(axisymmetric_density_model) :: density_model
        REAL*8, INTENT(OUT), DIMENSION(0:BASIS_LMAX, BASIS_NR) :: rho_l_out
        ...
        DO i_r = 1, BASIS_NR
            ...
            DO k = 1, n_mu
                ...
                DO l = 0, BASIS_LMAX, 2
                    ...
                    rho_l_out(l, i_r) = rho_l_out(l, i_r) + factor * rho1(1) * p(l)
                END DO
            END DO
        END DO

        DEALLOCATE(mu_q, w_q, p)
    END SUBROUTINE project_axisymmetric_density_to_table
    ...
    SUBROUTINE project_axisymmetric_density_generic(params, density_model)
        ...
        CALL project_axisymmetric_density_to_table(params, density_model, BASIS_RHO_L_GRID)
    END SUBROUTINE project_axisymmetric_density_generic

    SUBROUTINE compute_phi_tables_from_rho_input(rho_l_in, phi_l_out, dphi_l_dr_out)
        IMPLICIT NONE
        REAL*8, INTENT(IN),  DIMENSION(0:BASIS_LMAX, BASIS_NR) :: rho_l_in
        REAL*8, INTENT(OUT), DIMENSION(0:BASIS_LMAX, BASIS_NR) :: phi_l_out
        REAL*8, INTENT(OUT), DIMENSION(0:BASIS_LMAX, BASIS_NR) :: dphi_l_dr_out
        ...
        DO l = 0, BASIS_LMAX, 2
            ...
            DO i = 2, BASIS_NR
                ...
                r_hi = BASIS_R_GRID(i)
                f_lo = r_lo**(l+2) * rho_l_in(l, i-1)
                f_hi = r_hi**(l+2) * rho_l_in(l, i)
            ...
            DO i = BASIS_NR-1, 1, -1
                ...
                f_lo = r_lo**(1-l) * rho_l_in(l, i)
                f_hi = r_hi**(1-l) * rho_l_in(l, i+1)
                dr = r_hi - r_lo
                i_greater(i) = i_greater(i+1) + 0.5D0 * dr * (f_lo + f_hi)
            END DO

            DO i = 1, BASIS_NR
                phi_l_out(l, i) = prefactor * ( &
                    BASIS_R_GRID(i)**(-(l+1)) * i_less(i) + &
                    BASIS_R_GRID(i)**l * i_greater(i) )
                dphi_l_dr_out(l, i) = prefactor * ( &
    ...
    END SUBROUTINE compute_phi_tables_from_rho_input

    SUBROUTINE compute_phi_tables_from_rho()
        IMPLICIT NONE
        CALL compute_phi_tables_from_rho_input(BASIS_RHO_L_GRID, BASIS_PHI_L_GRID, BASIS_DPHI_L_DR_GRID)
        BASIS_EXPANSION_INITIALIZED = .TRUE.
    END SUBROUTINE compute_phi_tables_from_rho

    SUBROUTINE sphericalharmonicbasisforce_from_tables(n, x, y, z, phi_l_grid, dphi_l_dr_grid, ax, ay, az)
        IMPLICIT NONE
        INTEGER, INTENT(IN) :: n
        REAL*8, INTENT(IN), DIMENSION(n) :: x, y, z
        REAL*8, INTENT(IN), DIMENSION(0:BASIS_LMAX, BASIS_NR) :: phi_l_grid, dphi_l_dr_grid
        ...
        END DO
    END SUBROUTINE sphericalharmonicbasisforce_from_tables

    SUBROUTINE sphericalharmonicbasispotential_from_tables(n, x, y, z, phi_l_grid, phi_out)
        IMPLICIT NONE
        INTEGER, INTENT(IN) :: n
        REAL*8, INTENT(IN), DIMENSION(n) :: x, y, z
        REAL*8, INTENT(IN), DIMENSION(0:BASIS_LMAX, BASIS_NR) :: phi_l_grid
        ...
        DO i = 1, n
            ...
            phi_out(i) = phi_v
        END DO
    END SUBROUTINE sphericalharmonicbasispotential_from_tables
    ...
    SUBROUTINE sphericalharmonicbasisforce(n, x, y, z, ax, ay, az)
        ...
        REAL*8, INTENT(IN), DIMENSION(n) :: x, y, z
        REAL*8, INTENT(OUT), DIMENSION(n) :: ax, ay, az
        CALL sphericalharmonicbasisforce_from_tables(n, x, y, z, BASIS_PHI_L_GRID, BASIS_DPHI_L_DR_GRID, ax, ay, az)
    ...
    SUBROUTINE sphericalharmonicbasispotential(n, x, y, z, phi_out)
        ...
        REAL*8, INTENT(IN), DIMENSION(n) :: x, y, z
        REAL*8, INTENT(OUT), DIMENSION(n) :: phi_out
        CALL sphericalharmonicbasispotential_from_tables(n, x, y, z, BASIS_PHI_L_GRID, phi_out)
    END SUBROUTINE sphericalharmonicbasispotential

    SUBROUTINE sphericalharmonicbasisforce_component(component_index, n, x, y, z, ax, ay, az)
        IMPLICIT NONE
        INTEGER, INTENT(IN) :: component_index, n
        REAL*8, INTENT(IN), DIMENSION(n) :: x, y, z
        REAL*8, INTENT(OUT), DIMENSION(n) :: ax, ay, az

        IF (component_index < 1 .OR. component_index > BASIS_NCOMP) THEN
            ax = 0.0D0
            ay = 0.0D0
            az = 0.0D0
            WRITE(*,'(A)') "WARNING: sphericalharmonicbasisforce_component: invalid component_index"
            RETURN
        END IF

        CALL sphericalharmonicbasisforce_from_tables(n, x, y, z, BASIS_PHI_L_COMPONENT_GRID(:,:,component_index), &
                                                     BASIS_DPHI_L_DR_COMPONENT_GRID(:,:,component_index), ax, ay, az)
    END SUBROUTINE sphericalharmonicbasisforce_component

    SUBROUTINE sphericalharmonicbasispotential_component(component_index, n, x, y, z, phi_out)
        IMPLICIT NONE
        INTEGER, INTENT(IN) :: component_index, n
        REAL*8, INTENT(IN), DIMENSION(n) :: x, y, z
        REAL*8, INTENT(OUT), DIMENSION(n) :: phi_out
        ...
        IF (component_index < 1 .OR. component_index > BASIS_NCOMP) THEN
            phi_out = 0.0D0
            WRITE(*,'(A)') "WARNING: sphericalharmonicbasispotential_component: invalid component_index"
            RETURN
        END IF

        CALL sphericalharmonicbasispotential_from_tables(n, x, y, z, BASIS_PHI_L_COMPONENT_GRID(:,:,component_index), phi_out)
