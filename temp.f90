MODULE temp 
    ...
    ! let's add the king profile stuff

    SUBROUTINE KingDensityAsFunctionOfW(npoints,W,rho)
        REAL*8, INTENT(IN),DIMENSION(npoints) :: W
        REAL*8, INTENT(OUT), DIMENSION(npoints) :: rho
        REAL*8 :: x
        INTEGER :: i
        DO i = 1, npoints
            x = (i-1)*W/(npoints-1)
            rho(i) = 1.0d0/(1.0d0 + x*x)
        END DO 
    END SUBROUTINE KingDensityAsFunctionOfW
