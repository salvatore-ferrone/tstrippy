...
def test_cleargravity_resets_ncomp():
    g = tstrippy.gravity
    g.add_component("plummer", np.array([1e11, 2.0]))
    g.clear()
...
def test_addgravitycomponent_increments_ncomp():
    g = tstrippy.gravity
    g.add_component("plummer",   np.array([1e11, 2.0]))
    g.add_component("hernquist", np.array([5e10, 1.5]))
    assert g.gravity_ncomp == 2


def test_addgravitycomponent_invalid_model_stops():
    g = tstrippy.gravity
    g.add_component("bogusmodel", np.array([1.0, 2.0]))
...
def test_addgravitycomponent_wrong_nparams_stops():
    ...
    g.add_component("plummer", np.array([1e11, 2.0, 99.0]))
...
def test_exponential_oblate_halo_force_and_potential_are_finite():
    g = tstrippy.gravity
    g.add_component("exponentialoblatehalo", np.array([1.0e8, 5.0, 0.8]))
    g.finalize()
...
def test_exponential_disk_bessel_phi_even_and_az_odd_in_z():
    g = tstrippy.gravity
    g.add_component("exponentialdisk", np.array([5.0e8, 3.0, 0.3]))
    g.finalize()
